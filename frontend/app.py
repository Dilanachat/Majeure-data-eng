...
with st.sidebar:
    ...
    try:
        ...
        st.success(f"MLflow ✅  `{MLFLOW_URI}`")
        mlflow_ok = True
    except Exception:
        st.warning(f"MLflow ⚠️  non joignable")
        mlflow_ok = False
...
with tab_home:
    st.markdown("""
## 🏎️ Bienvenue sur le Dashboard F1 Pit Stop Prediction
...
### 🗂️ Contenu des onglets

| Onglet | Description |
|---|---|
| 🏎️ **Prédiction** | Saisir les données d'un tour et obtenir une prédiction instantanée via l'API |
| 📊 **Suivi du modèle** | Historique de tous les runs d'entraînement (MLflow) avec métriques et graphiques |
| 📈 **Évaluation** | Métriques détaillées du dernier modèle entraîné (F1, AUC-ROC, Precision, Recall) |
| 📋 **Table de prévisions** | Prédictions en batch sur un fichier CSV ou l'échantillon de données |
...
data/train.csv          ──►  src/feature.py  ──►  src/train.py  ──►  models/model.joblib
                                               ...
                                               src/api.py (FastAPI)
                                                        │
                                            frontend/app.py (Streamlit)  ◄── vous êtes ici
```
...
```bash
make mlflow          # 1. Serveur MLflow     → http://localhost:5000
make train MODEL=rf  # 2. Entraînement
make api             # 3. API FastAPI         → http://localhost:8000
make frontend        # 4. Ce dashboard        → http://localhost:8501
```
""")

    st.divider()
    col1, col2, col3 = st.columns(3)
    col1.metric("Dataset", "~440 000 tours", "F1 2022–2025")
    col2.metric("Modèles comparés", "3", "RF · XGBoost · LightGBM")
    col3.metric("Cible", "PitNextLap", "Classification binaire")
...
# ══════════════════════════════════════════════════════════════════════════════
with tab_pred:
    st.subheader("Prédiction d'un tour")
    ...
    with st.form("predict_form"):
        ...
        with col1:
            ...
            lap_number        = st.number_input("LapNumber",       min_value=1,    value=35)
            stint             = st.number_input("Stint",           min_value=1,    value=2)
            tyre_life         = st.number_input("TyreLife",        min_value=0,    value=20)
        ...
        with col2:
            lap_time_s        = st.number_input("LapTime (s)",              min_value=0.1, value=91.8)
            lap_time_delta    = st.number_input("LapTime_Delta",            value=0.4)
            cumul_deg         = st.number_input("Cumulative_Degradation",   min_value=0.0, value=1.5)
            race_progress     = st.slider("RaceProgress",   0.0, 1.0, 0.57)
            position_change   = st.number_input("Position_Change",          value=0)
            pit_stop          = st.selectbox("PitStop ce tour", [0, 1])
    ...
    if submitted:
        ...
        try:
            ...
            with col_res:
                if pred == 1:
                    st.success(f"### 🛞 PIT STOP prévu")
                else:
                    st.info(f"### ✅ Pas de pit stop")
            with col_gauge:
...
with tab_suivi:
    ...
    if not mlflow_ok:
        st.warning("MLflow n'est pas joignable. Lance `make mlflow` pour démarrer le serveur.")
        st.stop()
    ...
    if exp is None:
        ...
        if not runs:
            st.info("Aucun run enregistré. Lance `make train MODEL=rf` pour entraîner.")
        else:
            rows = []
            for r in runs:
                rows.append({
...
with tab_eval:
    ...
    if not mlflow_ok:
        ...
        if exp is None:
            ...
            if not runs:
                st.info("Aucun run. Lance `make train MODEL=rf`.")
            else:
                run     = runs[0]
                ...
                st.divider()
                st.markdown("**Seuils de validation**")
                f1_ok  = metrics.get("f1", 0) >= 0.50
                auc_ok = metrics.get("roc_auc", 0) >= 0.80
                st.write(f"F1 ≥ 0.50 : {'✅' if f1_ok else '❌'}  |  AUC-ROC ≥ 0.80 : {'✅' if auc_ok else '❌'}")
...
with tab_table:
    ...
    PAYLOAD_COLS = list(COL_MAP.values())
    ...
    if st.button("🔮 Lancer les prévisions", use_container_width=True):
        results = []
        errors  = 0
            ...
            for i, row in df_sample.iterrows():
                try:
                    ...
                    results.append({
                        ...
                        "TyreLife": int(row.get("TyreLife", 0)),
                        "RaceProgress": f"{float(row.get('RaceProgress', 0)):.0%}",
        ...
        if results:
            ...
            c2.metric("Pit stops prévus", n_pit)
