...
Seance 11 - TP Tests Donnees & Modele
    ...
    son seuil.
    Pre-requis : un modele enregistre au Model Registry via src.train.

Le jeu d'evaluation est logue comme dataset MLflow (tracabilite).

Lancement :
    python -m src.evaluate                          # derniere version du registry
    python -m src.evaluate --model-uri models:/F1-PitStop-Classifier/1
...
def latest_model_uri() -> str:
    ...
    if not versions:
        raise RuntimeError(
            ...
            "Lancez d'abord un entrainement : python -m src.train"
        )
...
def build_thresholds() -> dict[str, MetricThreshold]:
    ...
    """
    # S11-1 : seuils de qualite definis dans config.py
    return {
...
def evaluate_model(model_uri: str | None = None, validate: bool = True):
    ...
    with mlflow.start_run(run_name="evaluate"):
        # S11-2a : tracabilite — logger le jeu d'evaluation comme dataset MLflow
        ...
        # S11-2b : evaluation du modele (metriques + artefacts automatiques)
        ...
        # S11-3 : porte qualite — exception si un seuil n'est pas atteint
        if validate:
